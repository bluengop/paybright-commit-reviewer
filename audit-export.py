...
def write_commit_to_file(logger, fp, repository, commit, reviews):
...
def writeout(logger, fp, content:str):
    fp.write(content)
    logger.info(content)

def commit_report(logger, fp, github_api, repository, branch, required_reviews, startDate, endDate, csv_file, file_name):
    # Print headers
        ...
        while True:
            try:
                ...
                if pull_requests.totalCount == 0:
                    ...
                    write_commit_to_file(logger, fp, repository, commit, 0)
                    fp.flush()
                    index+=1
                else:
    ...
    writeout(logger, fp, f"Checksum: {file_checksum}\n")
    writeout(logger, fp, f"Records: {index}\n")
    writeout(logger, fp, f"Date: {datetime.now().strftime('%Y-%m-%d')}\n")
